...
### Option C: Manual Codex Worktree Windows (Recommended when the human wants to open each window manually) `[AI]`

Use when:
- the parent task already has child subtasks with dependencies
- the human wants worktrees created automatically but does **not** want background `codex exec ...` sessions
- each child window should begin only after the human opens a terminal and enters `$start ...`

#### Step 1: Prepare child worktrees from the main repo

```bash
python3 ./.trellis/scripts/multi_agent/prepare.py <parent-task> --platform codex
```

Notes:
- default mode is dependency-aware `ready-only`
- use `--all` to create all incomplete child worktrees up front
- the script writes `<parent-task>/worktree-launch.md` with exact terminal commands

#### Step 2: Human starts each child manually

Inside each prepared worktree:
...
#### Step 3: Merge completed child branches back on the main repo

```bash
python3 ./.trellis/scripts/multi_agent/merge.py <parent-task>
```

Optional cleanup:

```bash
python3 ./.trellis/scripts/multi_agent/merge.py <parent-task> --cleanup
```

Notes:
- real merges require the main repo to already be on the base branch with a clean working tree
- merge order follows the parent task `subtasks.depends_on` graph
- `--cleanup` uses existing cleanup behavior, including task archive + worktree removal
