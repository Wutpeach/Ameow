...
describe("mainWindowMotionBaseline", () => {
  it("captures the tuned Phase 2F panel visual timing contract", () => {
    ...
    expect(MAIN_WINDOW_MINIMIZED_ICON_SETTLE_SCALE_KEYFRAMES).toEqual([1, 1.025, 1]);
    expect(MAIN_WINDOW_MINIMIZED_ICON_SETTLE_SCALE_TIMES).toEqual([0, 0.7, 1]);
  ...
  it("captures the tuned Phase 2F minimized icon handoff timing contract", () => {
    ...
    expect(MAIN_WINDOW_MINIMIZED_ICON_SETTLE_TRANSITION).toEqual({
      duration: 0.28,
      times: [0, 0.7, 1],
      ease: [0.22, 1, 0.36, 1],
    });
