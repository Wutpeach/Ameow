...
describe("isTwitterXStatusUrl", () => {
  it("matches twitter and x status permalinks", () => {
    expect(isTwitterXStatusUrl("https://x.com/ameow/status/1234567890")).toBe(true);
    expect(isTwitterXStatusUrl("https://twitter.com/ameow/status/1234567890")).toBe(true);
  });

  it("rejects non-status X urls", () => {
    expect(isTwitterXStatusUrl("https://x.com/home")).toBe(false);
    expect(isTwitterXStatusUrl("https://example.com/ameow/status/1234567890")).toBe(false);
  });
});
...
describe("canonicalizeTwitterXPageUrl", () => {
  ...
  it("drops query and hash fragments from status urls", () => {
    expect(
      canonicalizeTwitterXPageUrl("https://twitter.com/ameow/status/1234567890?foo=1#bar"),
    ).toBe("https://twitter.com/ameow/status/1234567890");
  });
});
...
describe("shouldPreferTwitterXImageDrop", () => {
  it("prefers the image branch for X status drags that expose an image but no video hints", () => {
    expect(shouldPreferTwitterXImageDrop({
      dropUrl: "https://x.com/ameow/status/1234567890",
      html: `
        <a href="https://x.com/ameow/status/1234567890">
  ...
  it("does not prefer images when no extracted image url exists", () => {
    expect(shouldPreferTwitterXImageDrop({
      dropUrl: "https://x.com/ameow/status/1234567890",
      html: `<a href="https://x.com/ameow/status/1234567890">link</a>`,
      htmlImageUrl: null,
    })).toBe(false);
  });
});
