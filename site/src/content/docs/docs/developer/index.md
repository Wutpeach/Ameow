...
| Browser Lab | 属于 MR9 开发线，尚未进入 main | 不作为当前 stable 能力发布；待进入 authoritative baseline 后补充 |
| Presentation Lab | 同属 MR9 开发线，尚未进入 main | 同上 |

UI Lab 在开发环境中仍可通过 DEV-only 路由访问，但不建议依赖它构建长期工作流。Browser Lab 和 Presentation Lab 是计划中的开发线能力，当前 main 上不存在可调用的入口。
