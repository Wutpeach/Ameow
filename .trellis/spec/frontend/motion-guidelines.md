...
- Pointer enter/leave is the primary compact/full contract: entering the compact icon expands immediately, and leaving an unlocked full shell must collapse through the short leave grace window without waiting for the 3s idle timer. First launch is not exempt from this rule.
- Idle collapse and pointer-leave collapse have different hover semantics. `isPanelHovered` may guard interaction-driven collapse, but it must not prevent the idle timer from being armed or prevent an `"idle"` collapse request from compacting an otherwise unlocked full shell.
