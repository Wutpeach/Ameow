...
- `video_selected_v2` WebSocket requests must preserve all download-routing fields when converting to `queue_video_download`. This includes `pageUrl`, `selectionScope`, `clipStartSec`, `clipEndSec`, `videoUrl`, `videoCandidates`, `siteHint`, `title`, and quality preference fields. Do not manually rebuild this payload inline in `electron/main.mts`; use the shared queue-payload builder so new fields are testable.
...
- Good: YouTube/Bilibili injected player sends `video_selected_v2` with `clipStartSec` and `clipEndSec`; Electron bridge forwards those fields unchanged into `queue_video_download`.
...
- `electron/videoDownloadCommands.test.mts`: normal queue dispatch, injected `video_selected_v2` payload builder preserving clip fields, pasted assisted success, pasted assisted fallback, cancellation dispatch.
