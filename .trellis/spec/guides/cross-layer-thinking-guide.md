...
**Bad**: Assuming `window.ameow` will exist because the preload file compiles, without checking the BrowserWindow `webPreferences`, route boot mode, and renderer bootstrap path together.
...
### Mistake 11: Treating A Weak Drag Classification As More Authoritative Than A Later Canonical Note Hint

**Bad**: A waterfall-card drag starts with `mediaType: "image"` because only the cover image is visible locally, and later layers keep treating that first guess as final even after a higher-trust note-specific `detailUrl` or tokenized permalink becomes available.

**Good**: Give each hint an explicit trust order. Card-local DOM guesses are weak, note-linked/tokenized detail URLs are stronger, and resolved direct media URLs are strongest. Once a higher-trust note hint arrives, later layers must continue note-aware resolution before finalizing an image download.

### Mistake 12: Treating A Transparent-Window Flash As Renderer-Only Before Isolating Native Settle

**Bad**: A compact-window collapse flash appears right after the animation looks complete, and debugging stays focused on React state, Motion variants, icon ownership, or shell DOM layers without first disabling the post-animation native window-mode handoff.

**Good**: Split the flow into two contracts and test them independently:
- renderer visual motion (`motion.div`, AnimatePresence, content ownership)
- native settle after motion (`setIgnoreMouseEvents`, focusability changes, blur/focus transitions)

For transparent Electron windows, always ask:
- Does the flash disappear if the native settle is temporarily disabled?
- Which single native call reintroduces it (`setIgnoreMouseEvents`, `setFocusable`, `blur`)?
- Are we debugging a second BrowserWindow, or the same window changing native focus/input state?
...
- [ ] Verified drag payloads keep canonical note identity (`noteId`, tokenized `detailUrl`, source page URL, confidence) all the way through browser script -> extension background -> desktop runtime
...
- [ ] Verified a weak early classification does not short-circuit a later higher-trust canonical note hint (for example tokenized detail URL + medium/high video intent)
...
- [ ] Verified transparent-window animation flashes still reproduce when native post-animation settle is disabled; if not, isolate each native call before changing renderer motion
