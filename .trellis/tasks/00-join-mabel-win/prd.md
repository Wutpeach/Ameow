...
`Mabel-WIN` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/Mabel-WIN/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/Mabel-WIN/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee Mabel-WIN` to
...
python ./.trellis/scripts/task.py archive 00-join-mabel-win
```
