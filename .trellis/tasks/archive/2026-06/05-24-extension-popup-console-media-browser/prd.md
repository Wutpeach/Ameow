...
- Superseded scope note: `.trellis/tasks/05-24-extension-popup-options-page-redesign/` moves hidden-site list management out of the popup into an options page. The earlier `Sites` tab requirement in this PRD should not be implemented as the canonical hidden-site management surface unless that newer task is explicitly reversed.
