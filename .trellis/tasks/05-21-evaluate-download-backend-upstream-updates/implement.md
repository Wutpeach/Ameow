...
- [x] `douyin-dl` single post successful media download with valid Douyin session
...
- [x] `douyin-dl` 有效 Douyin session 真实媒体下载验证通过
...
- 已完成并验证：
  ...
  - Douyin site-session capture follow-up:
    - User reported that after scanning Douyin QR login and clicking Settings "确认保存", Windows prompted for an app to open a `bytedance://` URL and the Settings badge did not refresh to ready.
    - Root cause 1: site-session capture window loaded all popup/navigation URLs, including non-web custom schemes. `electron/main.mts` now blocks non-http(s) popup and top-level navigation URLs before Windows can invoke an external protocol handler.
    - Root cause 2: Douyin Settings readiness required `msToken`, but managed `douyin-dl`'s installed `CookieManager.validate_cookies()` only requires `ttwid`, `odin_tt`, and `passport_csrf_token`; `msToken` is optional and generated by upstream if absent. `src/site-sessions.ts` now aligns readiness with that backend contract.
    - Root cause 3: Douyin session smoke parsed a Netscape cookie file as both Netscape and Cookie-header text, which could create invalid tab-containing cookie keys and break generated YAML. `src/electron-runtime/douyinSession.ts` now rejects invalid header cookie names and filters invalid YAML keys defensively.
    - Local saved session check: `C:\Users\Administrator\AppData\Roaming\ameow\site-sessions\douyin.json` has 57 cookies, contains `ttwid` / `odin_tt` / `passport_csrf_token` and login markers `sessionid` / `sid_tt` / `sid_guard`, lacks `msToken`, and now reports `availability: "ready"`.
    - Regression coverage added:
      - `electron/siteSessionManager.test.mts` verifies Douyin ready state without `msToken` when backend-required cookies and a login marker are present.
      - `src/electron-runtime/douyinSession.test.ts` verifies Netscape lines are not misparsed as header tokens and invalid YAML keys are dropped.
    - Verification:
      - `npm test -- electron/siteSessionManager.test.mts src/electron-runtime/douyinSession.test.ts src/electron-runtime/douyinDlDownload.test.ts`
      - `npm run electron:build`
      - `npm run type-check`
      - `npm run lint`
      - `npm test`（103 个测试文件，609 个用例）
      - `git diff --check` 退出码 `0`，仅有 Windows 行尾转换提示
    - Current Douyin real-media smoke status:
      - Old sample URL `https://www.douyin.com/video/7493088730088770870` fails with `Failed to get video detail: 7493088730088770870`, so it was replaced as the default smoke sample.
      - `node ./scripts/smoke-douyin-dl-session-download.mjs --skip-build --url https://www.douyin.com/video/7642294524819131683` passed with the captured Ameow Douyin site session.
      - `node ./scripts/smoke-douyin-dl-session-download.mjs --skip-build` also passed after updating the default sample URL.
      - Output: `D:\Ameow\build\external-download-smoke\douyin-dl-session\央视网快看\2026-05-21_网传湖南怀化学院招收100多名印度留学生？官方辟谣_7642294524819131683.mp4`
      - Output size: `438817` bytes
      - This completes the valid Douyin session real-media downloader acceptance item.
- Release QA 后续项：
