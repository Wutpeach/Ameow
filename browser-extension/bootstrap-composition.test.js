...
const loadBackgroundWorker = async () => {
  ...
  return { sandbox, contextified, runtimeListeners, sockets, unhandled };
};
...
describe("background.js bootstrap composition", () => {
  it("evaluates the real service worker without an initialization-order ReferenceError", async () => {
    const { sandbox, runtimeListeners, sockets, unhandled } = await loadBackgroundWorker();
  ...
  it("preserves explicit opaque site hints in the generic transport", async () => {
    const { contextified } = await loadBackgroundWorker();
    const normalizeSiteHint = (value) =>
      vm.runInContext(`normalizeSiteHint(${JSON.stringify(value)})`, contextified);
    ...
    expect(normalizeSiteHint("https://evil.example/path")).toBeNull();
    expect(normalizeSiteHint("")).toBeNull();
    expect(normalizeSiteHint(undefined)).toBeNull();
  });
});
