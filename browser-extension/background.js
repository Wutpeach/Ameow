...
async function getCookiesForCandidateUrls(urls) {
  ...
  const cookieBlocks = await Promise.all(
    normalizedUrls.map((url) => getCookiesForUrl(url)),
  );
  const mergedCookieLines = new Map();
  let sawCookieHeader = false;

  for (const cookieBlock of cookieBlocks) {
    if (typeof cookieBlock !== 'string' || !cookieBlock.trim()) {
      continue;
    }

    for (const line of cookieBlock.split(/\r?\n/)) {
      ...
      mergedCookieLines.set(trimmed, line);
    }
  }

  if (mergedCookieLines.size === 0) {
    return '';
  }

  const header = sawCookieHeader
    ? '# Netscape HTTP Cookie File\n# https://curl.haxx.se/docs/http-cookies.html\n# This file was generated by Ameow\n\n'
    : '';
  return header + Array.from(mergedCookieLines.values()).join('\n');
}
...
async function handlePastedVideoSelectionResolveRequest(data) {
  ...
  try {
    ...
    const [cookies, qualityPreference] = await Promise.all([
      getCookiesForCandidateUrls([
        pageUrl,
        rawUrl,
        tabUrl,
      ]),
