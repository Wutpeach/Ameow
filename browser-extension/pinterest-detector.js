...
(function () {
  ...
  function createIconButton(className, title, onClick) {
    ...
    button.appendChild(createCatIconElement());
  ...
  function createDetailGroupSlot(templateSlot, onClick) {
    ...
    if (iconShell instanceof HTMLElement) {
      ...
      const catIcon = createCatIconElement();
      if (nativeSvg instanceof SVGElement) {
        catIcon.setAttribute("role", "img");
        catIcon.setAttribute("aria-hidden", "true");
        nativeSvg.replaceWith(catIcon);
      } else {
        iconShell.replaceChildren(catIcon);
      }
    } else {
      button.replaceChildren(createCatIconElement());
    }
