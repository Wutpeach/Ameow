...
```
Development Flow:
  Write code -> Test -> git commit -> $finish-work
                                       |
                                Wrap up + session recording
...
- `$record-session` - Legacy command name in older Trellis installs; current workflow wraps session recording into `$finish-work`
